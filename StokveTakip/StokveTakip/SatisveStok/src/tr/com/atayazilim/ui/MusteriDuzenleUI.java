...
public class MusteriDuzenleUI extends JDialog implements uiInterfaces {
	...
	public JPanel initPanel() {
		...
		panel.setBorder(BorderFactory.createTitledBorder("M��teri D�zenleme Sayfas�"));
		JPanel ustPanel = new JPanel(new GridLayout(6, 3));
		ustPanel.setBorder(BorderFactory.createTitledBorder("M��teri D�zenle"));
		
		JLabel musterilistLabel = new JLabel("M��teri Listesi",JLabel.RIGHT);
		fieldPanel.add(musterilistLabel);
		musterilistBox = new JComboBox(new MusteriDAL().GetAll2().toArray());
		fieldPanel.add(musterilistBox);
		JLabel idLabel = new JLabel("M��teri Id:",JLabel.RIGHT);
